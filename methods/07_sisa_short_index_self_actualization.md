...
key_file: "items/sisa_jones_crandall_ru_candidate_key.csv"
...
SISA сохранен как короткая возможная замена длинных методик самоактуализации. Сейчас это резерв, потому что русскоязычная психометрическая база и конкретная версия перевода требуют проверки.
