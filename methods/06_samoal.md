...
key_file: "items/samoal_lazukin_kalina_key.csv"
...
САМОАЛ используется как основной блок самоактуализации в актуальной батарее. Это самый длинный инструмент набора, поэтому его нельзя сокращать вручную: если методика включена, она переносится целиком и обрабатывается по ключу.

## Теоретическое основание

Методика относится к гуманистической психологии и POI-традиции Э. Шострома. Она измеряет широкую самоактуализацию личности, а не только профессиональную самореализацию.

## Источники и правовой статус

Основной источник адаптации: Калина Н.Ф., Лазукин А.В., 1998. Рабочий вторичный источник ключа: Dip-Psi. Полные формулировки пунктов в публичный репозиторий не включены из-за неясного правового статуса; в CSV хранится структура обработки.

## Пункты и ключ

- Карта пунктов: `items/samoal_lazukin_kalina_items.csv`.
- Ключ шкал: `items/samoal_lazukin_kalina_key.csv`.
...
raw_scale = sum(score(scale, item_i))
...
Общий показатель считать только после согласования способа агрегирования: как сумму нормированных шкал или как профиль без единого индекса.
...
- Правовой статус полного текста пунктов неясен.
