...
- применяет настройки оформления из `section_layout`;
- создает новую форму через API;
- добавляет вопросы из секций;
- раскладывает вопросы по страницам;
- автоматически делит длинные секции на несколько страниц, если для секции задано `answerable_items_per_page`;
- принудительно помечает все реальные вопросы как обязательные;
- оставляет информационные `comment`-блоки необязательными;
...
Для актуальной формы САМОАЛ делится на 4 страницы по 25 пунктов через настройку:

```json
"section_layout": {
  "samoal": {
    "answerable_items_per_page": 25
  }
}
```

Служебные заголовки продолжения блока в `exports/form_mapping.json` будут помечены полем `"synthetic": true`.
...
После загрузки нужно открыть форму в интерфейсе Яндекс.Форм и проверить внешний вид. Скрипт управляет структурой, страницами и обязательностью вопросов, но часть визуальных настроек темы может быть доступна только в интерфейсе.
