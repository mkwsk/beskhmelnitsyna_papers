...
- В `create_form.py` добавлена поддержка `section_layout`: можно переопределять вступительные comment-блоки без изменения самих секций.
- В `create_form.py` добавлена автоматическая разбивка длинной секции на несколько страниц через `answerable_items_per_page`.
- Для САМОАЛ задана разбивка на 4 страницы по 25 пунктов.
- В mapping-файл добавлено поле `synthetic` для служебных заголовков продолжения блока.
